...
\begin{frame}{Benchmarking Tools}
    \small
    \begin{itemize}
        \item \textbf{Dhrystone:} Measures CPU performance (DMIPS). \pause
        \item \textbf{CoreMark:} Focuses on integer performance, used in embedded systems benchmarking. \pause
        \item \textbf{Two configurations:}
        \begin{itemize}
            \item \textbf{Native Execution:} Benchmark runs as a normal process. \pause
            \item \textbf{Enclave Execution:} Benchmark runs within a Keystone enclave with memory isolation. \pause
        \end{itemize}
    \end{itemize}
    \vspace{0.5em}
    \uncover<5->{\textbf{Key Focus:} These tools provide insights into both CPU-bound and system-level performance.}
\end{frame}
...
\draw[fill=keystoneblue!60] (0,0) rectangle (0.4,0.4) node[right=0.5cm] {Actual region (contiguous)};
\draw[fill=warmaccent!60] (5,0) rectangle (5.4,0.4) node[right=0.5cm] {Actual region (fragmented)};
...
% Cloud
\node[box, fill=blue!20] (cloud) {\includegraphics[scale=0.05]{example-image} \\ \textbf{Cloud Server} \\ (Untrusted Environment)};

% Lock / Enclave
\node[box, fill=green!20, right=3cm of cloud] (enclave) {\faLock \\ \textbf{Keystone Enclave} \\ Secure Execution};
...
\begin{itemize}
    \item Performance optimizations with hardware extensions.
    \item Multi-enclave systems for more complex use cases.
    %\item Research on side-channel attacks and improved security measures.
    \item Development of better developer tools for enclave management.
\end{itemize}
\end{frame}
