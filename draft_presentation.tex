...
\begin{center}
\scalebox{1}{ % adjust this factor to shrink or enlarge
\begin{tikzpicture}[font=\scriptsize, >=latex, node distance=0.5cm]

% Base ISA
\node[draw, fill=blue!20, thick, minimum width=5cm, minimum height=0.8cm, align=center] (isa) {\textbf{RISC-V ISA} \\ Open, stable foundation};

% Optional Extensions layer
\node[draw, fill=yellow!30, thick, minimum width=4.5cm, minimum height=0.8cm, align=center, above=of isa] (ext) {Optional Extensions \\ (Vector, Crypto, Custom...)};

% Application domains
\node[draw, fill=orange!30, thick, rounded corners, minimum width=2.8cm, minimum height=0.8cm, align=center, above left=0.9cm and 0.4cm of ext] (embedded) {Embedded};
\node[draw, fill=green!30, thick, rounded corners, minimum width=2.8cm, minimum height=0.8cm, align=center, above=0.9cm of ext] (hpc) {HPC};
\node[draw, fill=purple!30, thick, rounded corners, minimum width=2.8cm, minimum height=0.8cm, align=center, above right=0.9cm and 0.4cm of ext] (enclave) {Enclaves};

% Arrows
\draw[->, thick] (isa) -- (ext);
\draw[->, thick] (ext.north west) .. controls +(0,0.4) and +(0,-0.4) .. (embedded.south);
\draw[->, thick] (ext.north) -- (hpc.south);
\draw[->, thick] (ext.north east) .. controls +(0,0.4) and +(0,-0.4) .. (enclave.south);
...
\node[anchor=south] at (0,1.1) {\textbf{Base}};
\node[anchor=south] at (12,1.1) {\textbf{Top}};

% Needed regions (non-contiguous)
\draw[fill=gray!30] (0.5,0.1) rectangle (2.2,0.9);
\node[font=\footnotesize] at (1.35,0.5) {\textbf{R1}};

\draw[fill=gray!30] (3.8,0.1) rectangle (4.6,0.9);
\node[font=\footnotesize] at (4.2,0.5) {\textbf{R2}};

\draw[fill=gray!30] (7.0,0.1) rectangle (9.1,0.9);
\node[font=\footnotesize] at (8.05,0.5) {\textbf{R3}};

% NAPOT allocations (outer boxes) and padding (hatched)
\draw[thick, fill=blue!20, draw=blue!80!black] (0.25,-0.3) rectangle (2.5,-1.1);
\node[font=\footnotesize, align=center] at (1.375,-0.7) {\textbf{NAPOT} \\ \footnotesize fit R1};
...
\draw[thick, fill=green!20, draw=green!60!black] (3.5,-0.3) rectangle (5.1,-1.1);
\node[font=\footnotesize, align=center] at (4.3,-0.7) {\textbf{NAPOT} \\ \footnotesize fit R2};
...
\draw[thick, fill=orange!20, draw=orange!70!black] (6.7,-0.3) rectangle (9.3,-1.1);
\node[font=\footnotesize, align=center] at (8.0,-0.7) {\textbf{NAPOT} \\ \footnotesize fit R3};
...
% Legend / note
\node[anchor=west] at (0,-1.6) {\footnotesize \textbf{Note:} Hatched = padding/waste due to power-of-two alignment};
