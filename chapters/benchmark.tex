...
rkloads, \texttt{coremark} stood out as the only one to exhibit consistent, positive scaling behavior. Performance improved by approximately 40.8\% with each additional core, and a similarly proportional uplift was observed with increased memory. This result is aligned with earlier observations: CoreMark’s mixed-instruction profile and parallelism-friendly design allow it to exploit both increased computational throughput and expanded memory capacity. As a compute-bound benchmark with moderate data locality, CoreMark scales predictably and efficiently with additional hardware provisioning.
...
% \centering
% \begin{threeparttable}
% \caption{ANOVA and Kruskal-Wallis results for hardware scaling effects. No statistically significant relationships found ($\alpha=0.05$).}
% \label{tab:anova-corr}
% \begin{tabular}{@{}l
...
% & \textbf{$R^2$} \\
% \midrule
% \texttt{coremark}      & 1.50 & 0.218 & 1.50 & 0.218 & 4.60 & 0.100 & 4.74 & 0.094 & 0.030 \\
% \texttt{coremark.ke}   & 0.01 & 0.975 & 0.01 & 0.975 & 0.02 & 0.989 & 0.00 & 1.000 & 0.000 \\
% \texttt{dhrystone}     & 0.28 & 0.837 & 0.28 & 0.837 & 0.71 & 0.870 & 0.65 & 0.883 & 0.005 \\
% \texttt{enclave.ke}    & 0.01 & 0.998 & 0.01 & 0.998 & 0.00 & 1.000 & 0.00 & 1.000 & 0.000 \\
% \texttt{kyber}         & 1.56 & 0.202 & 1.56 & 0.202 & 4.71 & 0.094 & 4.52 & 0.104 & 0.036 \\
% \texttt{kyber.ke}      & 0.00 & 1.000 & 0.00 & 1.000 & 0.00 & 1.000 & 0.00 & 1.000 & 0.000 \\
% \bottomrule
% \end{tabular}
% \begin{tablenotes}
% \footnotesize
% \item F: ANOVA F-statistic; H: Kruskal-Wallis H-statistic; p: p-value; $R^2$: coefficient of determination.
% \item (C): CPU core scaling; (M): memory scaling.
% \item Kruskal-Wallis used due to potential non-normality in small-sample performance data\footnote{Kruskal-Wallis is robust to non-normal distributions and unequal variances.}.
% \end{tablenotes}
% \end{threeparttable}
% \end{table}
...
\begin{threeparttable}
    ...
    \begin{axis}[
        ...
        ymin=0,
        ymax=5.0,
        ylabel={F / H statistic value},
        symbolic x coords={coremark,coremark.ke,dhrystone,enclave.ke,kyber,kyber.ke},
        xtick=data,
    ...
    % R² - crosshatch dots (dense)
    \addplot[pattern=crosshatch dots,pattern color=black] 
    coordinates {
    (coremark,0.030)   [0.030]
    (coremark.ke,0.000) []
    (dhrystone,0.005)  [0.005]
    (enclave.ke,0.000) []
    (kyber,0.036)      [0.036]
    (kyber.ke,0.000)   []
    };

    \legend{F (C),F (M),H (C),H (M),$R^2$}
    \end{axis}

    % Right axis for R² scaling
    \begin{axis}[
        hide axis,
        ymin=0, ymax=0.05,
        axis y line*=right,
    ...
    \item (C): CPU core scaling; (M): memory scaling.
    \item Kruskal–Wallis used due to potential non-normality in small-sample performance data\tnote{a}.
...
% \legend{Intercept,Cores Coef.,Memory Coef.}
% \end{axis}
% \end{tikzpicture}
% \caption{Linear regression coefficients of performance on hardware parameters. Positive values indicate performance gains; negative values indicate degradation.}
% \label{fig:regression-coefs-greyscale}
% \end{figure}
