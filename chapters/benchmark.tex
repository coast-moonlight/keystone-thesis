...
\addplot[pattern=dots,pattern color=black!50] 
    coordinates {(Kyber,29.2) (Dhrystone,24.7) (CoreMark,107.3)};
\legend{Native Time, Enclave Time} % only the left-axis series here
\end{axis}
...
\addplot[pattern=north east lines,pattern color=black] 
    coordinates {(Kyber,66000) (Dhrystone,66000) (CoreMark,66000)};
\legend{Involuntary CS} % right-axis series legend
\end{axis}

% --- Overhead factor labels ---
\begin{axis}[
    hide axis,
    width=14cm,
    height=8cm,
...
\node[font=\small, anchor=south] at (axis cs:Kyber,29.2) {2.73$\times$};
\node[font=\small, anchor=south] at (axis cs:Dhrystone,24.7) {1.61$\times$};
\node[font=\small, anchor=south] at (axis cs:CoreMark,107.3) {1.06$\times$};
\end{axis}

\end{tikzpicture}
\caption{Native vs enclave execution times (left axis) and involuntary context switches (right axis), with overhead factors above Enclave bars}
