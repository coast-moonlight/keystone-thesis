...
\begin{figure}[htbp]
    ...
    % Second Subfigure: Enclave Run10_SEQ vs Run5*
        ...
        \addplot[pattern=dots,pattern color=black] coordinates {
            (Total\ Time,107.02)
            (Iterations/Sec,929.80)
            (User\ CPU\ Time,41.16)
            (System\ CPU\ Time,41.00)
            (Max\ RSS,10.00)
            (Minor\ PF,1.00)
            (Major\ PF,0.001) % zero replaced by small non-zero
            (Voluntary\ CS,49.00)
        ...
        \end{axis}
        \end{tikzpicture}
        \caption{Log-scale comparison of key metrics for CoreMark 64M\_1Core: Native vs Enclave (Run10\_SEQ).}
