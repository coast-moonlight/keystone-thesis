...
Figure~\ref{fig:integrated_two_column_peripherals_below} provides a unified representation of the development workflow and the virtualized execution environment used in this thesis. On the left, the build process begins with the Keystone source code and proceeds through root filesystem generation via Buildroot, orchestration of key components through a \texttt{Make}-based system, and culminates in the launch of the full system within a QEMU-emulated RV64GC RISC-V platform. On the right, the runtime stack is depicted in detail: Ubuntu 22.04 LTS acts as the host operating system, supporting QEMU, which emulates the Keystone stack—including the Security Monitor, runtime, and enclave applications.
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
    font=\scriptsize,
...
% --- Left column: Build & Deployment Workflow ---
\node (src) [box] {Source Code \\ Keystone, Kernel, Apps};
\node (buildroot) [box, below=of src] {Buildroot \\ Rootfs + Kernel};
\node (make) [box, below=of buildroot] {Make Orchestration};
\node (launch) [box, below=of make] {QEMU Launch};
\node (bench) [box, below=of launch] {Run Enclave / Benchmark};

\foreach \from/\to in {src/buildroot, buildroot/make, make/launch, launch/bench} {
    \draw[arrow] (\from) -- (\to);
}

% --- Right column: Testbed Stack ---
\node (host) [stack, fill=gray!30, minimum height=1.1cm, right=5.0cm of src] {Ubuntu 22.04 LTS\\ Host OS};
\node (qemu) [stack, below=of host] {QEMU RV64GC Emulator};
\node (keystone) [stack, keystone, below=of qemu] {Keystone Stack\\ \footnotesize Sec. Monitor, Runtime, Enclaves};

% Peripherals BELOW Keystone, side-by-side
\node (virtio) [stack, fill=gray!20, below left=0.5cm and 0.5cm of keystone.south] {VirtIO Devices};
\node (plugin) [stack, fill=gray!20, below right=0.5cm and 0.5cm of keystone.south] {Cache Plugin};

% Testbed arrows
\draw[arrow] (host) -- (qemu);
\draw[arrow] (qemu) -- (keystone);
\draw[arrow] (keystone.south) -- ++(0,-0.25) -| (virtio.north);
\draw[arrow] (keystone.south) -- ++(0,-0.25) -| (plugin.north);

% Cross-links from workflow to stack (no intersection)
\draw[arrow, dashed] (launch.east) -- ++(0.8,0) |- (qemu.west);
\draw[arrow, dashed] (bench.east)  -- ++(1.2,0) |- (keystone.west);

% Annotation
\node[font=\tiny, anchor=south west] at ([xshift=0.15cm,yshift=0.15cm]host.north west) {SSH/GDB access};
